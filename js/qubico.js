$(document).ready(function() {
	...
	function resetAnimation(){
		...
		$('.text-slider .slides li.flex-active-slide').removeClass('animate');
	}
	
	if($('.jumbotron.bg-slider').length){
		
		var bgSlides = $('.jumbotron.bg-slider').data('slides').split(',');
		
		$('.jumbotron.bg-slider').backstretch(bgSlides, { fade: 500,duration:sliderSpeed});
		if($('.jumbotron .text-slider').length) $('.jumbotron.bg-slider').backstretch('pause');
	}
	
	$('.jumbotron .next-slide').click(function(){
		if($('.jumbotron .text-slider').length) {$('.jumbotron .text-slider').flexslider('next'); $('.jumbotron .text-slider').flexslider('play');}
		else if($('.jumbotron.bg-slider').length) $('.jumbotron.bg-slider').backstretch('next');
	})
	
	$('.jumbotron .prev-slide').click(function(){
